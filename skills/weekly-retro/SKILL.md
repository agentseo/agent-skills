...
## Subagent model (cost rule)

All "fast subagents" in this skill do **mechanical evidence-gathering** (read issue state, last commit/comment, find artifacts) — not reasoning. Spawn them with the Task tool pinned to a cheaper model: **`model: "claude-sonnet-4-6"`**. Do NOT let them silently inherit the session model (Opus) — that overpays for I/O-bound lookups across 5-10 issues. (The founder's interview and synthesis stay on the session model.)
...
3. For each outcome row (O1..ON) gather **evidence** in parallel via fast subagents (Task tool, `model: "claude-sonnet-4-6"`):
...
2. For **each** issue, a fast subagent (Task tool, `model: "claude-sonnet-4-6"`) gathers status evidence in parallel (5-10 issues per batch): what shipped, what didn't, last commit/comment, any blocker.
